...
const Messages = () => {
    ...
    const fetchChats = async () => {
        try {
            const response = await messageAPI.getRecentChats();
    ...
    const fetchConversation = async (otherUserId: number) => {
        try {
            ...
            const formatted = response.data.map((m: any) => ({
                id: m.id,
                sender: Number(m.senderId) === currentUserId ? 'admin' : 'user',
                text: m.content,
                            ...
                            >
                                {messagesList.map((msg) => (
                                    <div key={msg.id} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-3 ${msg.sender === 'admin'
                                            ? 'bg-yellow-400/20 border border-yellow-400/30'
