    ...
    const fetchMessages = useCallback(async () => {
        ...
        try {
            ...
            const messages = response.data.map((msg: any) => ({
                id: msg.id,
                sender: Number(msg.senderId) === currentUserId ? 'parent' : 'other',
                text: msg.content,
                                ...
                                chats.map((chat) => (
                                    <button
                                        ...
                                        className={`w-full text-left p-3 rounded-lg transition-colors ${selectedChat === chat.id
                                            ? 'bg-bright-sun-300/20 border border-bright-sun-200/30'
                            ...
                            >
                                {messagesList.map((msg) => (
                                    <div key={msg.id} className={`flex ${msg.sender === 'parent' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-3 ${msg.sender === 'parent'
                                            ? 'bg-bright-sun-300/20 border border-bright-sun-200/30'
